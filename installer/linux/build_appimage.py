"""Linux AppImage packaging builder for AM Workbench."""
...
ARTIFACT_NAME = "AMWorkbench.AppImage"
...
def build(staging_dir: Path | str, output_dir: Path | str) -> PreflightResult:
    ...
    appimagetool = _tool_path("appimagetool")
    if appimagetool:
        commands = ((appimagetool, str(staging_path), str(artifact_path)),)
        return _run_native_commands(commands, artifact_path=artifact_path, tool="appimagetool")

    return PreflightResult(
        False,
        ("cannot build .AppImage artifact because native AppImage tooling is missing: install appimagetool on PATH",),
        artifact_path=str(artifact_path),
    )


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build the Linux AppImage with appimagetool.")
