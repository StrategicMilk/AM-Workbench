...
The release matrix builds Windows CPU, Windows CUDA, Linux CPU, and Linux CUDA bundles. Every leg uses Rust 1.88.0 and CMake in Release mode. CUDA legs install CUDA Toolkit 12.4.1 through the commit-pinned `Jimver/cuda-toolkit` action and prove only that `nvcc`, CMake, Rust, and the CUDA-linked build path are available. A standard hosted runner has no qualifying GPU and therefore does not certify CUDA runtime behavior or model offload. Publishing adds a protected `cuda-certification` job routed to a repository-level self-hosted runner with the exact `self-hosted`, `vetinari-engine-gpu`, `linux`, `x64`, and `cuda-12-4` labels. Repository-level labels are required because the public repository is user-owned; GitHub runner groups are organization/enterprise resources. The scheduled training exit-hatch job uses the distinct `vetinari-training-gpu` custom label. The certification host must have one runner service, must not host a training runner, and must not share its persistent runner account or work directory with training.

The protected environment pins the immutable numeric runner ID and name as `ENGINE_CUDA_CERTIFICATION_RUNNER_ID` and `ENGINE_CUDA_CERTIFICATION_RUNNER_NAME`. It also pins the SHA-256 of the host machine ID, service account, and runner work directory as `ENGINE_CUDA_HOST_ID_SHA256`, `ENGINE_CUDA_SERVICE_ACCOUNT`, and `ENGINE_CUDA_WORK_DIRECTORY`, and stores a dedicated fine-grained `ENGINE_RUNNER_AUDIT_TOKEN` with repository Administration read access. The binding job uses its ordinary same-run `github.token` with job-level Actions read permission for the attempt-specific Workflow Jobs endpoint; the fine-grained audit token is used only for the repository runner inventory. The root-owned, digest-pinned `/opt/vetinari/bin/amw-engine-cuda-certifier` receives the exact same-run Linux CUDA archive and per-leg manifest as separate mandatory `--archive` and `--manifest` inputs, plus the pinned GGUF fixture. It emits schema-version-2 raw evidence containing device identity, a positive device-memory delta for actual model offload, the positive integer `workflow_run_attempt`, and host attestation for the machine ID, service account, work directory, runner-service count, and absence of a training runner. Its signed `subject` contains the exact `amw-engine-linux-cuda.zip` and `manifest-linux-cuda.json` names, byte sizes, and lowercase SHA-256 digests. Before writing the result, the certifier signs the UTF-8 serialization of the complete schema-version-2 object excluding `certifier_signature`, with keys sorted lexically, no insignificant whitespace (`","` and `":"` separators), and non-ASCII characters emitted directly. The signature uses ECDSA P-256/SHA-256 and a host-protected, non-exportable TPM, PKCS#11, or equivalent private key. `certifier_signature` contains exactly `algorithm: "ecdsa-p256-sha256"`, the lowercase SHA-256 of the DER public key as `public_key_sha256`, and the DER ECDSA signature as canonical base64 in `value_base64`. `ENGINE_CUDA_CERTIFIER_PUBLIC_KEY_DER_BASE64` pins the corresponding DER SubjectPublicKeyInfo bytes in the protected environment.

A separate `cuda-certification-binding` job runs on a GitHub-hosted runner after the GPU job. It independently downloads the same-run `engine-linux-cuda` artifact, requires its flat two-file shape, compares both files to the signed subject, verifies the per-leg manifest binds the archive, and verifies the certifier signature against the protected public key. It then queries the attempt-specific workflow-job API record and the live repository runner inventory. It rejects pagination or malformed metadata and requires the successful workflow job and live runner registration to match the protected numeric ID, name, Linux OS, and exact five-label set. It then binds the authenticated raw result into schema-version-3 certification evidence. Release assembly compares the subject to the actual archive and build manifest again. The protected `publication-prerequisites` job downloads only that same-run, same-attempt bound artifact and independently re-verifies the signature plus every protected runner and host authority value before release work can proceed. Replayed, rewritten, or fabricated raw JSON, substituted archives, unbound results, a deleted and re-registered same-name runner, a second local runner service, a training-eligible host, or a substituted runner ID cannot reach publication. Editing the committed blocked example cannot unlock release. CPU legs require only the hosted runner's CMake and pinned Rust toolchain.
