<script>
  ...
  import { PlanStatus } from '$lib/contracts/enums.js';
  ...
  let planIsExecuting = $derived(
    planStatus === PlanStatus.EXECUTING || planStatus === PlanStatus.RUNNING || planStatus === PlanStatus.IN_PROGRESS
  );
  let planIsPaused = $derived(planStatus === PlanStatus.PAUSED);
  let planIsTerminal = $derived(
    planStatus === PlanStatus.COMPLETED || planStatus === PlanStatus.COMPLETE ||
    planStatus === PlanStatus.FAILED || planStatus === PlanStatus.CANCELLED
  );
  ...
  async function handleDecompose(e) {
    ...
    try {
      ...
      planStatus = result?.status ?? (activePlanId ? PlanStatus.EXECUTING : null);
      planStatusError = '';
...
<div class="plan-builder-view">
  ...
  {:else}
    ...
    <div class="tab-bar" role="tablist" aria-label="Plan builder sections">
      {#each [
        { id: 'decompose', icon: 'cut', label: 'Decompose' },
        { id: 'result', icon: 'sitemap', label: 'Plan Tree' },
        { id: 'dod', icon: 'check-double', label: 'DoD/DoR' },
      ...
      <div id="panel-decompose" role="tabpanel" aria-labelledby="tab-decompose" class="panel-content">
        <div class="decompose-layout">
          <section class="card decompose-form-card">
            <h3 class="section-title"><Icon name="bullseye" /> Goal</h3>
          ...
          {#if Object.keys(knobs).length > 0}
            <aside class="card knobs-card" aria-label="Decomposition knobs">
              <h3 class="section-title"><Icon name="sliders-h" /> Knobs</h3>
        ...
        {:else}
          <section class="card result-card" aria-label="Decomposition result">
            <div class="result-header">
              <h3 class="section-title"><Icon name="sitemap" /> Plan Tree</h3>
              <div class="result-meta">
                ...
                {#if planStatus && !planIsTerminal}
                  <span class="plan-status-badge plan-status-{planIsPaused ? 'paused' : 'running'}">
                    <Icon name={planIsPaused ? 'pause' : 'play'} />
                ...
                {/if}
                {#if planStatus === PlanStatus.COMPLETED || planStatus === PlanStatus.COMPLETE}
          ...
          <div class="dod-layout">
                ...
                <ul class="checklist" aria-label="Definition of Done checklist">
                  {#each (Array.isArray(dodDor.definition_of_done) ? dodDor.definition_of_done : [dodDor.definition_of_done]) as item, i (i)}
                    <li class="checklist-item">
                      <Icon name="circle-check" class="check-icon" />
                      <span>{item}</span>
                    </li>
                  {/each}
                </ul>
            ...
            {#if dodDor.definition_of_ready}
              <section class="card dor-card" aria-label="Definition of Ready">
                <h3 class="section-title">
                  <Icon name="play-circle" class="text-primary" />
                  Definition of Ready
                </h3>
