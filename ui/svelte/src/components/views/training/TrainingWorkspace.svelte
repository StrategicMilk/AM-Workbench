...
<div class="training-view">
  <div class="view-header">
    <h2>
      <Icon name={kaizenFocus ? 'seedling' : 'graduation-cap'} />
      {kaizenFocus ? 'Kaizen' : 'Training'}
    </h2>
    <HelpPopover
      title={kaizenFocus ? 'Kaizen workflow' : 'Training'}
  ...
  {#if loading}
    <div class="loading-state" role="status" aria-live="polite">
      ...
      Loading {kaizenFocus ? 'kaizen workflow' : 'training status'}...
    </div>
  {:else}
    {#if kaizenFocus}
      <section id="kaizen-workflow" class="card kaizen-workflow-card" aria-label="Kaizen workflow actions">
        <div class="card-header">
          <h3><Icon name="seedling" /> PDCA Workflow</h3>
          <span class="status-badge status-success" role="status" aria-label="Kaizen workflow primary status" aria-live="polite">Primary</span>
        </div>
        <div class="kaizen-actions-grid">
          <div>
            <strong>Plan</strong>
            <span>Review weekly report counts and the current kaizen score.</span>
          </div>
          <div>
            <strong>Do</strong>
            <span>Run `vetinari kaizen gemba` to inspect friction in the current execution flow.</span>
          </div>
          <div>
            <strong>Check</strong>
            <span>Compare confirmed, failed, and reverted improvements before promotion.</span>
          </div>
          <div>
            <strong>Act</strong>
            <span>Promote changes only after the review evidence is visible in the history below.</span>
          </div>
        </div>
      </section>
    {/if}
    <div class="training-layout">
      ...
      <div class="training-main">
        ...
        <section class="card status-card" aria-label="Training status">
          <div class="card-header">
            <h3><i class="fas fa-circle-notch" aria-hidden="true"></i> Status</h3>
        ...
        <section class="card config-card" aria-label="Training configuration">
          <div class="card-header">
            <h3><Icon name="sliders-h" /> Configuration</h3>
            <button
              class="btn-icon"
            ...
            >
              <Icon name={showConfigForm ? 'chevron-up' : 'chevron-down'} />
            </button>
          </div>
        ...
        <section class="card synthetic-card" aria-label="Synthetic data generation">
          <div class="card-header">
            <h3><Icon name="robot" /> Synthetic Data</h3>
            <button
              class="btn btn-secondary btn-sm"
        ...
        <section class="card history-card" aria-label="Training run history">
          <div class="card-header">
            <h3><Icon name="history" /> History</h3>
                ...
                <tbody>
                  {#each history as run (run.id ?? run.run_id)}
                    ...
                    >
                      ...
                      <td class="evidence-refs">{run.evidence_refs?.length ? run.evidence_refs.join(', ') : 'no evidence'}</td>
                      <td>
                        <button
                          class="btn-icon"
                        ...
                        >
                          <Icon name={expandedRunId === (run.id ?? run.run_id) ? 'chevron-up' : 'chevron-down'} />
                        </button>
                      </td>
                    </tr>
                      ...
                      <tr class="run-detail-row" aria-label="Run details">
                        <td colspan="8">
                          <dl class="run-detail-grid">
                            ...
                            <dt>Evidence</dt><dd>{run.evidence_refs?.length ? run.evidence_refs.join(', ') : 'no evidence'}</dd>
                            {#if run.error}
      ...
      <aside class="training-sidebar" aria-label="Idle training and kaizen stats">
        <section class="card idle-card">
          <div class="card-header">
            <h3><Icon name="moon" /> Idle Training</h3>
            {#if idleStats != null}
        ...
        <section class="card kaizen-card">
          <div class="card-header">
            <h3><Icon name="chart-line" /> Kaizen Metrics</h3>
          </div>
...
<style>
  ...
  .kaizen-actions-grid div {
    min-width: 0;
    border: 1px solid var(--border-muted, #334155);
    border-radius: 8px;
    padding: 12px;
    background: var(--surface-subtle, #111827);
  }

  .kaizen-actions-grid strong,
  .kaizen-actions-grid span {
    display: block;
  }
  ...
  .evidence-refs { font-family: var(--font-mono); font-size: 0.75rem; overflow-wrap: anywhere; }
  ...
  @media (max-width: 1024px) {
    ...
    .kaizen-actions-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
  }

  @media (max-width: 640px) {
    .kaizen-actions-grid { grid-template-columns: 1fr; }
  }
</style>
